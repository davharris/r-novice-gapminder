...
contents?  There are many good options.
...
> ## Tip: Logical operations {.callout}
>
...
> right. 
>
>  * `!` logical NOT: converts `TRUE` to `FALSE` and `FALSE` to `TRUE`. It can go before
>    a single condition (eg `!TRUE` becomes `FALSE`), or before a whole vector of conditions and convert
>    each element (eg `!c(TRUE, FALSE)` becomes `c(FALSE, TRUE)`).
>  * `&&` and `||` compare **only** the first element of two vectors and ignore the 
>    remaining elements.
>
> Additionally, you can compare the elements within a single vector using the `all` function
> (which returns `TRUE` if every element of the vector is `TRUE`) and the `any` function
> (which returns `TRUE` if one or more elements of the vector are `TRUE`).
